...
def icsCreateAndSave():  # 写成ics文件并保存ics
...
def CreateTime():
    ...
    # DONE_EventUID = random_str(20) + "&zzh"
...
def uniteSetting():
    ...
    DONE_ALARMUID = random_str(30) + "&zzh"
    #
    global DONE_UnitUID
    DONE_UnitUID = random_str(20) + "&zzh"
    print("uniteSetting")
...
def setClassInfo():  # 打开class info文件
    data = []
    with open('conf_classInfo.json', 'r', encoding='GBK') as f:
        data = json.load(f)
    global classInfoList
...
def checkInput(checkType, input):
    if (checkType == checkFirstWeekDate):
        if (checkFirstWeekDate(input)):
            info = "输入有误，请重新输入第一周的星期一日期(如：20210905):\n"
            firstWeekDate = input(info)
            checkInput(checkFirstWeekDate, firstWeekDate)
        else:
